...
Responsibilities:
1. Create and configure FastAPI application instance
...
def create_app(static_dir: Optional[Path] = None) -> FastAPI:
    ...
    Args:
        static_dir: Static files directory path (optional, defaults to 'static' under project root)
    ...
    app = FastAPI(
        ...
        description=(
            "A-share/HK/US Stock Intelligent Analysis System API\n\n"
            "## Feature Modules\n"
            "- Stock Analysis: Trigger AI intelligent analysis\n"
            "- History: Query historical analysis reports\n"
            "- Stock Data: Retrieve market data\n\n"
            "## Authentication\n"
            "Supports optional runtime authentication (enable/disable via WebUI settings page)"
        ),
        version="1.0.0",
    ...
    if has_frontend:
        ...
        async def root():
            """Root route - return frontend page"""
...
<p class="status">API Version 1.0.0 &bull; <a href="/api/health">/api/health</a></p>
        ...
        @app.get("/", include_in_schema=False)
        async def root():
            """Root route - return guide page when frontend is not built"""
    ...
    @app.get(
        ...
        description="Used by load balancers or monitoring systems to check service status"
    )
    ...
    if has_frontend:
        ...
        async def serve_spa(request: Request, full_path: str):
            """SPA route fallback - return index.html for non-API routes"""
...
# Default application instance (for direct use by uvicorn)
app = create_app()
