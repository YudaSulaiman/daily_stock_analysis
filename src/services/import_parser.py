...
_CODE_ALIASES = frozenset({"code", "Stock code", "代码", "stock_code", "symbol"})
_NAME_ALIASES = frozenset({"name", "Stock name", "名称", "stock_name"})
...
def _parse_dataframe(df: pd.DataFrame) -> List[Tuple[Optional[str], Optional[str], str]]:
    ...
    for _, row in df.iterrows():
        ...
        if not code and name_val:
            code = resolve_name_to_code(name_val)
            if not code:
                logger.debug(f"[ImportParser] 名称解析Failed: {name_val}")
...
def parse_import_from_bytes(data: bytes, filename: Optional[str] = None) -> List[Tuple[Optional[str], Optional[str], str]]:
    ...
    logger.debug(f"[ImportParser] Started解析文件: filename={filename or '-'}, ext={ext or '-'}, bytes={len(data)}")
    ...
    if ext == ".xlsx" or looks_like_zip:
        try:
            ...
            if looks_like_zip:
                ...
                raise ValueError(f"Excel 解析Failed: {e}。{hint}") from e
    ...
    try:
        ...
        raise ValueError(
            f"CSV 解析Failed：请检查分隔符是否一致、列数是否匹配。"
...
def parse_import_from_text(text: str) -> List[Tuple[Optional[str], Optional[str], str]]:
    ...
    logger.debug(f"[ImportParser] Started解析粘贴文本: bytes={len(text.encode('utf-8'))}")
