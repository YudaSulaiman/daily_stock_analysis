...
6. **工具Failed处理** — 记录Failed原因，使用已有数据继续分析，不重复调用Failed工具。
...
5. **工具Failed处理** — 记录Failed原因，使用已有数据继续分析，不重复调用Failed工具。
...
class AgentExecutor:
    ...
    def _build_user_message(self, task: str, context: Optional[Dict[str, Any]] = None) -> str:
        ...
        parts.append("\n请使用可用工具获取缺失的数据（如历史K线、新闻等），然后以决策仪表盘 JSON 格式输出Analysis result。")
        return "\n".join(parts)
