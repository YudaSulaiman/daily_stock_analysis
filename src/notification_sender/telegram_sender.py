...
class TelegramSender:
    ...
    def __init__(self, config: Config):
        ...
        self._pinned_this_run: bool = False
    ...
    def _try_pin_first_of_run(self, message_id: Optional[int]) -> None:
        """若启用且本次运行尚未置顶，则尝试置顶。无论成功与否都标记为已尝试，避免同次运行重复调用 API。"""
        if message_id is None or self._pinned_this_run:
            return
        if not self._telegram_config.get('pin_first_message'):
            return
        self._pinned_this_run = True
        if not self._pin_message(message_id):
            logger.warning('Telegram 置顶失败（请检查 Bot 是否拥有"置顶消息"权限）')
        ...
        for attempt in range(1, max_retries + 1):
            ...
            if response.status_code == 200:
                ...
                if result.get('ok'):
                    ...
                    self._try_pin_first_of_run(message_id)
                    return True
                else:
