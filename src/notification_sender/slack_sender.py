...
class SlackSender:
    ...
    def send_to_slack(self, content: str) -> bool:
        ...
        try:
            ...
            logger.error(f"分割 Slack 消息Failed: {e}, 尝试整段发送。")
            chunks = [content]
    ...
    def _send_slack_webhook(self, content: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 200 and response.text == "ok":
                logger.info("Slack Webhook 消息发送Succeeded")
                return True
            logger.error(f"Slack Webhook 发送Failed: HTTP {response.status_code} {response.text[:200]}")
            return False
        except Exception as e:
    ...
    def _send_slack_bot(self, content: str) -> bool:
        ...
        try:
            ...
            if result.get("ok"):
                logger.info("Slack Bot 消息发送Succeeded")
                return True
            logger.error(f"Slack Bot 发送Failed: {result.get('error', 'unknown')}")
            return False
        except Exception as e:
    ...
    def _send_slack_image(self, image_bytes: bytes, fallback_content: str = "") -> bool:
        ...
        Args:
            ...
            fallback_content: 图片发送Failed时的回退文本
        ...
        if self._use_bot:
            ...
            try:
                ...
                result3 = resp3.json()
                if result3.get("ok"):
                    logger.info("Slack Bot 图片发送Succeeded")
                    return True
                logger.error("Slack Completed上传Failed: %s", result3.get('error', 'unknown'))
            except Exception as e:
                logger.error("Slack Bot 图片发送异常: %s", e)

        # Webhook 模式或 Bot 上传Failed：回退为文本
        if fallback_content:
            logger.info("Slack 图片不支持或Failed，回退为文本发送")
            return self.send_to_slack(fallback_content)

        logger.warning("Slack 图片发送Failed，且无回退内容")
        return False
